...
const Pagination = ({length, limit, page}:{length:number; limit:number; page:number;}) => {
    ...
    return(
        <div className="mt-16 flex justify-center w-full gap-8 text-blue-600">
            <button className={Number(page) === 1 ? "text-blue-100 cursor-not-allowed" : ""} onClick={()=>changePage(1)}>
            ...
            <button className={Number(page) === 1 ? "text-blue-100 cursor-not-allowed" : ""} onClick={()=>changePage(Number(page)-1)}>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="size-5">
            ...
            <button className={Number(page) === pages ? "text-blue-100 cursor-not-allowed" : ""} onClick={()=>changePage(Number(page)+1)}>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="size-5">
                    <path fillRule="evenodd" d="M8.22 5.22a.75.75 0 0 1 1.06 0l4.25 4.25a.75.75 0 0 1 0 1.06l-4.25 4.25a.75.75 0 0 1-1.06-1.06L11.94 10 8.22 6.28a.75.75 0 0 1 0-1.06Z" clipRule="evenodd" />
                </svg>
            </button>
            <button className={Number(page) === pages ? "text-blue-100 cursor-not-allowed" : ""} onClick={()=>changePage(pages)}>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="size-5">
