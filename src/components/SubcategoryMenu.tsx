...
const SubcategoryMenu = ({tabId} : any) => {
    return (
        <div className="absolute z-10 top-36 bg-white flex flex-wrap w-full">
            {menus[tabId-1].map((option)=>(
                <Link href={option.link} className="w-1/12 h-16 px-4 hover:bg-gray-100 flex items-center justify-center text-center">{option.title}</Link>
            ))}
        </div>
    )
}
