    ...
    return(
        <div className="flex flex-col gap-2 sm:w-[calc(50%-1rem)] lg:w-[calc(33.33%-1.3334rem)] w-full">
                ...
                <div className="flex flex-col gap-2 max-h-[5.5rem] overflow-y-auto">
                    {categories.map((category)=>(
                        <div key={category} className="flex gap-1">
                            ...
                            <button type="button" className="text-gray-400 hover:text-red-500 transition-colors duration-200" onClick={()=>removeCategory(category)}>
            ...
            <div className="flex gap-2">
                <button type="button" className={"transition-colors duration-200 hover:text-blue-700 text-gray-400 w-max flex gap-1 " + (!(categories.length > 0) && "my-4")} onClick={()=>{setCount(count+1); addCategory(count);}}>
                ...
                {categories.length > 3 && (
                    <button type="button" className="text-red-200 flex gap-1 items-center hover:text-red-500 transition-colors duration-200" onClick={()=>{setCategories([]); setFinal(undefined);}}>
