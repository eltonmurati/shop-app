...
const DoubleRange = ({title, measurement}:{title:string; measurement:string;}) => {
    ...
    return (
        <div className="flex flex-col gap-4 bg-bwcgray rounded-2xl py-2 px-2 h-max cursor-pointer" onClick={()=>(setOpen(!open))}>
            <h3 className="text-xs font-medium text-center px-2">{title}</h3>
            {open && (
                <>
                    <div className="w-44 bg-white h-2 relative rounded-full -mt-1" onClick={(e)=>(e.stopPropagation())}>
                        <input type="range" name="min_val" min="0" max="100" value="50" 
                    ...
                    <div className="flex justify-between">
                        <div className="flex w-20" onClick={(e)=>(e.stopPropagation())}>
