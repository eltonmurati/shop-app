...
const AddProductSpecs = () => {
    ...
    return(
        <div className="flex flex-col gap-2 w-full max-w-[20rem]">
                    ...
                    {specs.map((spec)=>(
                        <div key={spec} className="flex gap-1">
                            <div className="flex gap-2">
                                <input required type="text" name={"speckey"+spec} placeholder="Key" className="text-sm h-6 w-1/2 ring-2 ring-inset ring-gray-300 rounded-md px-2 outline-none"></input>
            ...
            <button type="button" className={"hover:text-blue-700 text-gray-400 w-max flex gap-1 " + (!(specs.length > 0) && "my-4")} onClick={()=>{setCount(count+1); addSpec(count);}}>
