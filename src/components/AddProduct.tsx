...
const AddProduct = ({open, onClose}:{open:boolean; onClose:()=>void;}) => {
    ...
    return(
        <div className={"fixed top-0 left-0 h-full w-full p-4 flex justify-center items-center " + (fadeout ? "opacity-0 animate-[fadeout_200ms_linear]" : "animate-[fadein_200ms_linear]")}>
                ...
                <form className="flex flex-col gap-8 justify-between max-h-full overflow-hidden" onSubmit={handleSubmit}>
                    <div className="flex flex-wrap gap-8 max-h-full overflow-auto">
                        <div className="flex flex-col gap-2 w-full">
                            <label htmlFor="image" className="text-sm text-gray-700">Images</label>
                        ...
                        <div className="flex gap-8 sm:w-[calc(50%-1rem)] lg:w-[calc(33.33%-1.3334rem)] w-full">
                            <div className="flex flex-col gap-2">
                                ...
                                <input type="checkbox" name="sale" className="transition-colors linear duration-200 appearance-none checked:bg-blue-700 h-14 w-14 cursor-pointer ring-2 ring-inset ring-gray-300 rounded-md accent-blue-700" onChange={e=>setSale(e.target.checked)} />
                            </div>
                            {sale && (
                                <div className={"flex flex-col gap-2 w-full"}>
